 ...
 * Date: 6/13/16
 */
public class CyclicPreviousPlayerInitializerStrategy implements PlayerInitializerStrategy {
	@Override
	public Optional<PlayerInitializer> determine(List<PlayerInitializer> playerInitializers, PlayerInitializer currentPlayerInitializer) {
		...
		return currentPlaybackIndex >= 0
				? Optional.of(playerInitializers.get((currentPlaybackIndex - 1 + playerInitializers.size()) % playerInitializers.size()))
				: Optional.absent();
	}
}
